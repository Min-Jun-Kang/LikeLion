...
namespace Day9
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            //var sortedNames = names.OrderBy(n => n); //오름차순으로으로 정렬
            ...
            ////매서드 구문
            //var sortedMethod = nums.OrderBy(n => n);//오름차순 정렬
            //var sortedMethod2 = nums.OrderByDescending(n => n);//내림차순 정렬
            ...
            //var lengths = words.Select(w => w.Length);//문자열의 길이로 변환
            //var UpperWords = words.Select(w => w.ToUpper());//배열 안의 단어를 대문자로 변환
            ...
            //Console.WriteLine($"Average: {avg:F2}");// 소수 둘째자리까지 표현
            ...
            //foreach (var d in data)
            //{
            //    if (Math.Abs(d - target) < Math.Abs(nearest - target))//절대값으로 비교
            //        nearest = d;
            //}

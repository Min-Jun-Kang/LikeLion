        ...
        static void Main(string[] args)
        {
            ...
            while (isAlive) 
            {
                ...
                if (input == 1)
                {
                    ...
                    int eventChance = rand.Next(1, 101); //1~100 사이 랜덤 이벤트 발생

                    if (eventChance <= 30) //30% 전투 발생
                    {
                        int damage = rand.Next(5, 21);
                        Console.WriteLine($" ⚔️ 몬스터를 만났습니다! (체력 -{damage})");
                        health -= damage;
                    }
                    else if (eventChance <= 70) //40% 보상
                    {
                        int reward = rand.Next(100, 301); //100~300 사이 랜덤
                        Console.WriteLine($" 💰 보물을 발견했습니다! (+{reward} 골드)");
                        gold += reward;
                    }
                    else //30% 체력 회복
                    {
                        int heal = rand.Next(10, 31); //10~30 사이 랜덤
